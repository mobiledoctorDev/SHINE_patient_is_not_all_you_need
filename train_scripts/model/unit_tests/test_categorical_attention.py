...
from mc_scripts.model.attention_model import CategoricalAtentionMultiplyIntensityModel, CategoricalAttentionAddIntensityModel
...
def test_2_train_on_epoch(sample_israel_dataset):
    ...
    train_result_info = CategoricalAtentionMultiplyIntensityModel.train_on_epoch(0, data_loader, model, loss_func, optimizer)
    ...
    # **************** Train Epoch 1 ****************
    data_loader = IsraelDataLoader(sample_israel_dataset, batch_size=8, shuffle=True)
    train_result_info = CategoricalAtentionMultiplyIntensityModel.train_on_epoch(1, data_loader, model, loss_func, optimizer)
...
def test_3_val_on_epoch(sample_israel_dataset):
    ...
    result = CategoricalAtentionMultiplyIntensityModel.validation_on_epoch(0, model, data_loader, val_metrics)
...
def test_4_adding_model_validation(sample_israel_dataset):
    ...
    model = CategoricalAttentionAddIntensityModel(
        dict_category, d_model=d_model, n_head=n_head, len_seq=train_cat.shape[1]
    )
    model.eval()
    val_metrics = {
...
def test_5_adding_model_train_on_epoch(sample_israel_dataset):
    ...
    # **************** Train Epoch 0 ****************
    data_loader = IsraelDataLoader(sample_israel_dataset, batch_size=8, shuffle=True)
    train_result_info = CategoricalAttentionAddIntensityModel.train_on_epoch(0, data_loader, model, loss_func, optimizer)
    logger.debug(f"epoch 0 train finished. train_result_info:{train_result_info}")
    ...
    # **************** Train Epoch 1 ****************
    data_loader = IsraelDataLoader(sample_israel_dataset, batch_size=8, shuffle=True)
    train_result_info = CategoricalAttentionAddIntensityModel.train_on_epoch(1, data_loader, model, loss_func, optimizer)
    logger.debug(f"epoch 1 train finished. train_result_info:{train_result_info}")
...
def test_6_adding_model_val_on_epoch(sample_israel_dataset):
    ...
    result = CategoricalAttentionAddIntensityModel.validation_on_epoch(0, model, data_loader, val_metrics)
    logger.info(f"result:{result}")
