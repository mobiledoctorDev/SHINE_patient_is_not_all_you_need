...
from mc_scripts.model.attention_model import CategoricalTransformerEncoderMultiplyIntensity, CategoricalTransformerEncoderAddIntensity
...
def test_1_validation(sample_israel_dataset):
    ...
    logger.info(f"sample_israel_dataset.dataset_np_cat:{sample_israel_dataset.dataset_np_cat.shape}")
    ...
    model = CategoricalTransformerEncoderMultiplyIntensity(
        ...
        len_seq=sample_israel_dataset.dataset_np_cat.shape[1],
    )
...
def test_2_train_on_epoch(sample_israel_dataset):
    ...
    train_result_info = CategoricalTransformerEncoderMultiplyIntensity.train_on_epoch(0, data_loader, model, loss_func, optimizer)
    ...
    # **************** Train Epoch 1 ****************
    data_loader = IsraelDataLoader(sample_israel_dataset, batch_size=8, shuffle=True)
    train_result_info = CategoricalTransformerEncoderMultiplyIntensity.train_on_epoch(1, data_loader, model, loss_func, optimizer)
...
def test_3_val_on_epoch(sample_israel_dataset):
    ...
    data_loader = IsraelDataLoader(sample_israel_dataset, batch_size=8, shuffle=True)
    result = CategoricalTransformerEncoderMultiplyIntensity.validation_on_epoch(0, model, data_loader, val_metrics)
    logger.info(f"result:{result}")

def test_4_add_intensity_validation(sample_israel_dataset):
    train_gt = torch.FloatTensor(sample_israel_dataset.dataset_np_gt)
    train_cat = torch.LongTensor(sample_israel_dataset.dataset_np_cat)
    train_intensity = torch.FloatTensor(sample_israel_dataset.dataset_np_intensity)
    logger.info(f"sample_israel_dataset.dataset_np_cat:{sample_israel_dataset.dataset_np_cat.shape}")
...
def test_5_add_intensity_train_on_batch(sample_israel_dataset):
    train_gt = torch.FloatTensor(sample_israel_dataset.dataset_np_gt)
    train_cat = torch.LongTensor(sample_israel_dataset.dataset_np_cat)
    train_intensity = torch.FloatTensor(sample_israel_dataset.dataset_np_intensity)
    train_input = (train_cat, train_intensity)
...
def test_4_add_intensity_train_on_epoch(sample_israel_dataset):
    train_gt = torch.FloatTensor(sample_israel_dataset.dataset_np_gt)
    train_cat = torch.LongTensor(sample_israel_dataset.dataset_np_cat)
    train_intensity = torch.FloatTensor(sample_israel_dataset.dataset_np_intensity)
    train_input = (train_cat, train_intensity)
    ...
    # **************** Train Epoch 0 ****************
    data_loader = IsraelDataLoader(sample_israel_dataset, batch_size=8, shuffle=True)
    train_result_info = CategoricalTransformerEncoderAddIntensity.train_on_epoch(0, data_loader, model, loss_func, optimizer)
    logger.debug(f"epoch 0 train finished. train_result_info:{train_result_info}")
    ...
    # **************** Train Epoch 1 ****************
    data_loader = IsraelDataLoader(sample_israel_dataset, batch_size=8, shuffle=True)
    train_result_info = CategoricalTransformerEncoderAddIntensity.train_on_epoch(1, data_loader, model, loss_func, optimizer)
    logger.debug(f"epoch 1 train finished. train_result_info:{train_result_info}")
...
def test_5_add_intensity_val_on_epoch(sample_israel_dataset):
    train_gt = torch.FloatTensor(sample_israel_dataset.dataset_np_gt)
    train_cat = torch.LongTensor(sample_israel_dataset.dataset_np_cat)
    train_intensity = torch.FloatTensor(sample_israel_dataset.dataset_np_intensity)
    train_input = (train_cat, train_intensity)
    ...
    data_loader = IsraelDataLoader(sample_israel_dataset, batch_size=8, shuffle=True)
    result = CategoricalTransformerEncoderAddIntensity.validation_on_epoch(0, model, data_loader, val_metrics)
    logger.info(f"result:{result}")
